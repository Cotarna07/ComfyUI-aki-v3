...
当前项目已经具备外部 pipeline 原型：长图切片、PaddleOCR provider、OCR-based Dialogue provider、lightweight detection provider、Qwen3-VL director 的 Ollama / local / OpenAI-compatible 接入、Stage 3A / 4A / 5 / 6 gate、ComfyUI batch submitter、`template_patcher` 节点参数注入，以及 `agent-skills/comfyui/workflows/02-project/manga-anime-pipeline/` 下的项目级 API workflow 模板和 mapping。Stage 1 章节分析闭环已经开始按工作流边界拆到 `pipeline/workflows/chapter_analysis/`，旧 `pipeline/stage1.py` 仅保留兼容入口。当前仍需继续强化的是 Grounded-SAM-2 真实检测 provider、Manga Image Translator provider、ComfyUI 自建节点包、各 route 模板的真实视觉验收与高阶动作/表情链路。
...
1. 技能层已有 API 资产：放在 `agent-skills/comfyui/workflows/01-shared/`，例如现有 Wan、LTX、VACE、Qwen Image 等导出或测试工作流。这些文件用于复用和参考，不直接等同于漫画 pipeline 的最终 route 模板。
2. 项目级 route 模板：放在 `agent-skills/comfyui/workflows/02-project/manga-anime-pipeline/`，由 Stage 6 按 `workflow_route` 读取并提交。这里的文件名必须和 `configs/comfy.default.json` 的映射一致。
...
```text
agent-skills/comfyui/workflows/02-project/manga-anime-pipeline/
...
推荐基底：Wan 2.2 I2V 或 LTX I2V。现有技能注册表中的 `wan22_i2v_api`、`ltx2_i2v_api` 可作为导出和参数参考，但项目模板仍需保存为 `agent-skills/comfyui/workflows/02-project/manga-anime-pipeline/dialogue_light_motion.json`。
...
推荐基底：WanAnimate、LTX Pose-to-Video 或带动作参考的 I2V 工作流。现有技能注册表中的 `ltx2_pose_to_video_api` 和 `agent-skills/comfyui/workflows/01-shared/wan22_animate_pose_compat_test.json` 可作为参考，但生产模板必须重新整理成项目 route 模板。
...
1. `agent-skills/comfyui/workflows/02-project/manga-anime-pipeline/` 负责保存 Stage 6 可提交的项目级 API workflow 模板和 mapping。
...
这一阶段的限制也很明确：Stage 6 已具备模板、mapping、`template_patcher` 和 batch submitter 注入框架，但 route 模板仍需按真实 ComfyUI 环境逐条视觉验收；detection 还不是完整 Grounded-SAM-2 生产链，自建节点也还没落地。
...
2. 先生成 `agent-skills/comfyui/workflows/02-project/manga-anime-pipeline/dialogue_light_motion.json` 和对应 mapping，接 Wan 2.2 I2V 或 LTX I2V。
