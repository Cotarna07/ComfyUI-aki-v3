...
- 用户明确要求：**不能因为设备性能而降低最终输出质量，模型、VAE、LoRA 与工作流均以最佳效果优先；需要更多算力时可以租用云 GPU。** 本地降尺寸、量化或快速模型仅可作为清楚标记的预览/诊断结果，不是最终质量门槛，也不是云端高质量路线的前置条件。
...
4. 视频阶段优先采用锁定优秀静帧内容的确定性镜头运动和后期光效；若使用生成式视频模型，必须以逐帧主体稳定性检查为准，并在云端采用效果最好的最终配置。
...
下文中早期写到的 `RTX 4080` 来自工作区历史说明，不应再作为本轮实际设备判断依据。`RTX 5070 Ti 16GB` 只决定哪些预览能在本机执行，不决定最终质量上限；高质量方案需要更多算力时，直接使用云端执行。
...
| 12–16GB（当前本地） | `Flux.1 Kontext FP8` 静态预览基线、SDXL + 风格 LoRA 创意对照 | 仅用于诊断、提示词与构图筛选，不作为最终质量上限 |
...
前述步骤可用于验证方向；最终成片质量不以本地设备为上限，优选流程超过本机能力时应在云 GPU 上完成。
