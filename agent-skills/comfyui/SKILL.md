...
- 用户要求质量优先时，目标配置必须按预期最终效果、结构一致性和任务适配度选择，不得仅因当前本机显存或速度改用明显较差的最终模型或参数。
- 本地可使用 fp8/量化、较小同系模型或受控尺寸完成诊断、A/B 预览与流程排障；必须记录模型、精度、尺寸、采样、提示词与质量短板，并清楚标记其不代表最终质量。
- 如果效果更好的模型、VAE、LoRA、控制节点或高分辨率流程需要更多算力，应记录完整高质量配置并转用云 GPU 执行，不得要求其先在本机降级跑通才进入候选。
...
- 显存不足：预览/诊断可临时降尺寸或采用较轻配置并明确标记；质量优先的最终路线应迁移到足够显存/算力的环境执行，不得默认沿用降质参数。
