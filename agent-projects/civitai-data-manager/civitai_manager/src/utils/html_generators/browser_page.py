...
def build_preview_markup(output_dir, base_name):
    sanitized_base_name = sanitize_filename(base_name)
    model_dir = Path(output_dir) / sanitized_base_name
    preview_candidates = []

    for pattern in (
        f"{sanitized_base_name}_preview*.jpg",
        f"{sanitized_base_name}_preview*.jpeg",
        f"{sanitized_base_name}_preview*.png",
        f"{sanitized_base_name}_preview*.webp",
        f"{sanitized_base_name}_preview*.mp4",
        f"{sanitized_base_name}_preview*.webm",
    ):
        preview_candidates.extend(sorted(model_dir.glob(pattern)))

    if not preview_candidates:
        return ''

    preview_path = f"{sanitized_base_name}/{preview_candidates[0].name}"
    preview_suffix = preview_candidates[0].suffix.lower()

    if preview_suffix in {'.mp4', '.webm'}:
        mime_type = 'video/mp4' if preview_suffix == '.mp4' else 'video/webm'
        return (
            '<video class="model-cover model-cover-video" muted loop playsinline preload="metadata">'
            f'<source src="{preview_path}" type="{mime_type}">'
            'Your browser does not support the video tag.'
            '</video>'
        )

    return (
        f'<img class="model-cover model-cover-image" src="{preview_path}" '
        'onerror="if (this.src.includes(\'preview_0\')) { this.src = this.src.replace(\'preview_0\', \'preview_1\'); } '
        'else { this.style.display=\'none\'; }" loading="lazy">'
    )
    ...
    try:
        ...
        for model_type, models in sorted(models_by_type.items()):
            ...
            for model in models:
                ...
                preview_markup = build_preview_markup(output_dir, model['base_name'])
