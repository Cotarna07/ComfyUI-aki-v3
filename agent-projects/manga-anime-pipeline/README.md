...
| ComfyUI 投递 | `configs/comfy.default.json` | — | ComfyUI 服务（默认 `http://127.0.0.1:8188`）+ `../../agent-skills/comfyui/workflows/02-project/manga-anime-pipeline/*.json` 模板 |
...
# 同时启动 ComfyUI 服务 http://127.0.0.1:8188，并准备好 ../../agent-skills/comfyui/workflows/02-project/manga-anime-pipeline/*.json 工作流模板
```
...
4. ComfyUI 工作流模板：把对应 API workflow JSON 放到 `../../agent-skills/comfyui/workflows/02-project/manga-anime-pipeline/`，文件名要与 `configs/comfy.default.json` 中的 `workflow_templates` 映射一致（`establish_scene.json`、`dialogue_light_motion.json` 等）。
