...
| Director（Qwen3-VL） | `configs/stage1.full.director.json` | `qwen3vl` / `qwen3-vl` / `qwen_vl` | `requirements-director.txt`（torch + transformers + accelerate）+ `D:\AI\models\vl\qwen3-vl` 权重 |
...
# 同时把 Qwen3-VL 权重放到 D:\AI\models\vl\qwen3-vl，并在 configs/stage1.full.director.json 中确认 model_path
...
| Stage 5 | `scripts/run_stage5_gate.py` | 上面三份配置 | `stage5_gate_report.{json,md}` | torch/transformers/模型权重缺失 | workflow_route 非法、空 prompt、score 越界 |
...
- `blocked`：真实依赖缺失（PaddleOCR、Pillow/opencv、torch/transformers、ComfyUI 服务、Qwen3-VL 权重、ComfyUI 工作流模板）。处理：按 `next_action` 安装依赖或准备资源后重跑。
...
2. Qwen3-VL：把权重放在 `D:\AI\models\vl\qwen3-vl`（或修改 `configs/stage1.full.director.json` 的 `director.qwen3vl.model_path`）。
