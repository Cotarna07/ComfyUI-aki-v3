...
- 对用户明确要求“效果最好”或“质量优先”的图像/视频生成任务，最终交付必须以视觉质量、结构一致性与任务适配度优先；不得仅因当前本机性能、显存、耗时或运行便利而选择明显更低质量的模型、精度、尺寸或流程。
- 本地测试可以采用较小尺寸、量化权重或加速配置进行诊断、构图筛选与流程验证，但必须明确标记为“预览/诊断，非最终质量”，并记录参数、效果和限制。
- 若最佳质量路线超出当前设备承载能力，应直接记录所需模型、精度、工作流与算力规格，并转用用户可提供的云 GPU 执行高质量复测或最终制作；不得把本机可运行性作为高质量路线的准入前提。
- 选择工作流、主模型、VAE、LoRA、控制模型和放大/补帧模型时，优先选择预期效果最好的兼容资源；速度、显存和成本仅用于安排执行环境与预览方式，不作为降低最终质量的理由。
